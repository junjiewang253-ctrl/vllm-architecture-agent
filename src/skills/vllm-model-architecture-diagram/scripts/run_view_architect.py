...
def build_architecture_view_graph(architecture_concept: dict[str, Any], fact_graph: dict[str, Any]) -> dict[str, Any]:
    ...
    pages = [
        _page(
            ...
            [
                _edge("overview_input_to_embedding", "data_flow", "input", "embedding", "", [c_exec], exec_facts),
                _edge("overview_embedding_to_decoder", "data_flow", "embedding", "decoder_stack", "", [c_exec], exec_facts),
                _edge("overview_decoder_to_norm", "data_flow", "decoder_stack", "final_norm", "", [c_exec], exec_facts),
                _edge("overview_norm_to_hidden", "data_flow", "final_norm", "hidden_states", "", [c_exec], exec_facts),
                _edge("overview_hidden_to_logits_processor", "data_flow", "hidden_states", "logits_processor", "", [c_exec], exec_facts),
                _edge("overview_lm_head_to_logits_processor", "dependency", "lm_head", "logits_processor", "weights", [c_exec], exec_facts, show_label=True),
                _edge("overview_logits_processor_to_logits", "data_flow", "logits_processor", "logits", "", [c_exec], exec_facts),
            ],
        ),
        _page(
            "attention",
            ...
            [
                ...
                _node("q_path", "data", "Q Path", "query stream", [c_attn], attn_facts, "attention_data", 0, 3),
                _node("k_path", "data", "K Path", "key stream", [c_attn], attn_facts, "attention_data", 1, 3),
                _node("v_path", "data", "V Path", "value stream", [c_attn], attn_facts, "attention_data", 2, 3),
                _node("qk_processing", "process", "Q/K Processing", "optional Q/K norm", [c_qk], attn_facts, "attention_process", 0, 4),
                _node("rope", "process", "RoPE Processing", "rotary embedding", [c_qk], attn_facts, "attention_process", 1, 4),
                _node("kv_cache_boundary", "storage", "KV Cache Boundary", "external cache behavior", [c_kv], attn_facts, "storage_boundary", 2, 4),
            ...
            [
                _edge("attention_input_to_qkv", "data_flow", "attention_hidden_states", "qkv_projection", "", [c_attn], attn_facts),
                _edge("attention_qkv_to_split", "data_flow", "qkv_projection", "qkv_split", "", [c_attn], attn_facts, target_port="in"),
                _edge("attention_split_to_q", "data_flow", "qkv_split", "q_path", "Q", [c_attn], attn_facts, source_port="q", show_label=True),
                _edge("attention_split_to_k", "data_flow", "qkv_split", "k_path", "K", [c_attn], attn_facts, source_port="k", show_label=True),
                _edge("attention_split_to_v", "data_flow", "qkv_split", "v_path", "V", [c_attn], attn_facts, source_port="v", show_label=True),
                _edge("attention_q_to_qk", "data_flow", "q_path", "qk_processing", "", [c_qk], attn_facts),
                _edge("attention_k_to_qk", "data_flow", "k_path", "qk_processing", "", [c_qk], attn_facts),
                _edge("attention_qk_to_rope", "data_flow", "qk_processing", "rope", "", [c_qk], attn_facts),
                _edge("attention_rope_to_backend", "data_flow", "rope", "attention_backend", "", [c_backend], attn_facts),
                _edge("attention_v_to_backend", "data_flow", "v_path", "attention_backend", "", [c_backend], attn_facts),
                _edge("attention_kv_cache_to_backend", "boundary", "kv_cache_boundary", "attention_backend", "external KV cache", [c_kv, c_backend], attn_facts, show_label=True),
                _edge("attention_backend_to_output_projection", "boundary", "attention_backend", "output_projection", "backend output", [c_attn, c_backend], attn_facts, show_label=True),
                _edge("attention_output_projection_to_output", "data_flow", "output_projection", "attention_output", "", [c_attn], attn_facts),
            ],
        ),
        _page(
            "moe",
            ...
            [
                ...
                _node("router", "process", "Router", "GateLinear + top-k", [c_moe], moe_facts, "moe_process", 0, 1),
                _node("fused_moe", "component", "FusedMoE", "external fused experts", [c_moe], moe_facts, "moe_component", 0, 2),
                _node("routed_experts", "component", "Routed Experts", "local / physical experts", [c_moe], moe_facts, "moe_component", 0, 3),
                _node("shared_experts", "component", "Shared Experts", "optional shared MLP", [c_moe], moe_facts, "moe_component", 1, 3),
                _node("expert_parallel", "capability", "Expert Parallel", "EP group + EPLB", [c_parallel], moe_facts, "parallel_capability", 1, 2),
                _node("moe_output", "data", "Output", "restored hidden states", [c_moe], moe_facts, "runtime_data", 0, 4),
            ],
            [
                _edge("moe_input_to_router", "data_flow", "moe_hidden_states", "router", "", [c_moe], moe_facts),
                _edge("moe_router_to_fused", "data_flow", "router", "fused_moe", "router logits", [c_moe], moe_facts, show_label=True),
                _edge("moe_fused_to_routed_experts", "data_flow", "fused_moe", "routed_experts", "", [c_moe], moe_facts),
                ...
                _edge("moe_fused_to_output", "data_flow", "fused_moe", "moe_output", "", [c_moe], moe_facts),
            ],
        ),
        _page(
            "parallel_strategy",
        ...
        _page(
            ...
            [
                ...
                _node("qkv_weights", "data", "q_proj / k_proj / v_proj", "packed attention weights", [c_checkpoint], checkpoint_facts, "weight_data", 0, 1),
                _node("qkv_proj", "component", "qkv_proj", "packed vLLM parameter", [c_checkpoint, c_attn], checkpoint_facts, "weight_target", 0, 2),
                _node("gate_up_weights", "data", "gate_proj / up_proj", "stacked FFN weights", [c_checkpoint], checkpoint_facts, "weight_data", 1, 1),
                _node("gate_up_proj", "component", "gate_up_proj", "stacked vLLM parameter", [c_checkpoint], checkpoint_facts, "weight_target", 1, 2),
                _node("expert_weights", "data", "Expert Weights", "expert checkpoint tensors", [c_checkpoint, c_moe], checkpoint_facts, "weight_data", 2, 1),
                _node("expert_params", "component", "Expert Params", "FusedMoE parameters", [c_checkpoint, c_moe], checkpoint_facts, "weight_target", 2, 2),
                _node("vllm_params", "storage", "vLLM Parameters", "loaded model state", [c_checkpoint], checkpoint_facts, "checkpoint_target", 1, 3),
            ],
            [
                _edge("checkpoint_hf_to_qkv", "mapping", "hf_checkpoint", "qkv_weights", "", [c_checkpoint], checkpoint_facts),
                ...
                _edge("checkpoint_qkv_to_params", "mapping", "qkv_proj", "vllm_params", "", [c_checkpoint], checkpoint_facts),
                _edge("checkpoint_gate_up_to_params", "mapping", "gate_up_proj", "vllm_params", "", [c_checkpoint], checkpoint_facts),
                _edge("checkpoint_expert_params_to_params", "mapping", "expert_params", "vllm_params", "", [c_checkpoint], checkpoint_facts),
            ],
        ),
        _page(
            "vllm_boundary",
            ...
            [
                _edge("boundary_adapter_to_attention", "boundary", "hyv3_model", "vllm_attention", "delegates", [c_backend], boundary_facts, show_label=True),
                ...
                _edge("boundary_adapter_to_moe", "boundary", "hyv3_model", "vllm_fused_moe", "delegates", [c_moe], boundary_facts, show_label=True),
                _edge("boundary_weights_to_loader", "boundary", "local_weight_mapping", "vllm_auto_loader", "delegates", [c_checkpoint], boundary_facts, show_label=True),
    ...
    return {
        ...
        "legend": [
            {"type": "data_flow", "label": "Runtime tensor/data flow"},
